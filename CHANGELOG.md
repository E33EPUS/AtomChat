...
- **打开设置页时直接停在底部**：`ScrollController` 的「首帧吸底」原本是给聊天记录用的（打开聊天要立刻看到最新消息），但根页面的设置 / 资料列表与设置子页复用了同一个控制器，设置页一打开就滚到底部。现在控制器带显式锚点：聊天列表保持底部锚点，设置与资料列表使用顶部锚点，首次渲染与 `reset()` 后都停在顶部。
- 三端同源：以上修复在 Fabric 1.21.1 / NeoForge 1.21.1 / Forge 1.20.1 同步实现，单测三端各 492 项全绿。
...
- 三端同源：以上修复在 Fabric 1.21.1 / NeoForge 1.21.1 / Forge 1.20.1 同步实现，单测三端各 488 项全绿。
...
- 首次将 AtomChat 0.2.3 移植到 NeoForge 1.21.1（ModDevGradle 2.0.141，NeoForge 21.1.235，官方 Mojang 映射）。功能与 Fabric 0.2.3 对齐。

### NeoForge Port

- First NeoForge 1.21.1 port of AtomChat 0.2.3 (ModDevGradle 2.0.141, NeoForge 21.1.235, official Mojang mappings). Feature parity with Fabric 0.2.3.
