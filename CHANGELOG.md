...
- **通知音效听不到**：`PositionedSoundInstance.master` 参数是 `(音效, 音量, 音高)`，旧代码把音量传到了音高位，实际音量固定 0.25；现已按配置音量播放。
...
- **Notification sound was nearly inaudible**: `PositionedSoundInstance.master` takes `(sound, volume, pitch)`; the old call passed the volume into the pitch slot. It now plays at the configured volume.
