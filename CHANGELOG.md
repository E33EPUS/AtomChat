...
- 细调：服务端下发的表情格与本地格一样有悬停高亮（只是没有删除按钮）；服务端常用的颜色与本地一致（不再暗一档），只靠「本服」分组标题与缺失的按钮区分；服务器没有任何可下发内容时，两个面板都不出现服务端相关行。
...
- 三端同源：Forge 1.20.1 / Fabric 1.21.1 / NeoForge 1.21.1 同步实现，单测三端各 455 项。
