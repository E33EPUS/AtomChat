...
[![Build](https://github.com/E33EPUS/AtomChat/actions/workflows/build.yml/badge.svg?branch=NeoForge-1.21.1)](https://github.com/E33EPUS/AtomChat/actions/workflows/build.yml)
...
- **消息气泡**：左右分栏、真实玩家名 + 皮肤头像、引用胶囊、时间戳分隔、系统消息胶囊、防刷屏合并、紧凑消息分组
- **图片消息**：CICode 图片发送/显示/保存，接收开关（关闭时显示绿色占位）
- **交互**：跨消息文字拖选 + Ctrl+C 复制、可点击 span（tell/坐标/链接）、右键菜单（气泡/头像/玩家卡）、头像双击 poke
- **输入**：多行输入 + 行内选择、表情 / 颜文字 / 自制表情面板、常用语、@提及、WATUT 输入中指示
- **缓存治理**：聊天图片压缩缓存自动清理，旧数据目录自动迁移，设置 → 关于可查看并清空缓存
