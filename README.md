...
<p align="center">
  <em>为 MC 原版聊天框带来手机聊天 App 的体验</em>
</p>
...
5. 右键任意消息可 **复制** 或 **引用回复**；单击头像打开玩家档案页，双击头像触发 QQ 式戳一戳动画，右键头像呼出菜单（@ 提及 / 私聊 / 传送 / 屏蔽）
...
- 🧠 **消息捕获** — 真实玩家身份从 MessageHandler 的三个通道中捕获；未知消息会降级为系统文本
...
- 🔔 **通知横幅与音效** — 被 @、被引用、收到私聊时，面板顶部弹出横幅（发送者头像 + 类型 + 名字 + 内容预览），点击跳转原消息并高亮；配套原创合成的气泡提示音，可在 设置 → 聊天 →「通知」分别开关并调音量
- 🖼️ **图片消息** — 原生渲染 `[[CICode]]`，并与 E33Chat / ChatImage 系列模组互通；图片保持宽高比，支持右键保存
- 📤 **发送本地图片** — 可通过图片按钮选取、拖拽到窗口内，或使用 Ctrl+V 粘贴；选取器以「详细信息」视图打开，并显示内联缩略图
- 😊 **表情 / 颜文字 / 表情包** — 三个标签页带有滑动指示器和全宽推入切换动画；表情包存储在本地（png/jpg/jpeg，最多 10 个）
...
- 🧭 **@ / 私聊 / 传送 / 屏蔽** — 右键头像呼出菜单，可 @ 提及、跳转私聊、快速传送与屏蔽；单击头像则进入个人档案
- 🪢 **私聊** — 聊天列表列出当前在线玩家，单击进入聊天界面，右键呼出菜单（个人档案 / 传送 / 屏蔽）
- 👥 **在线玩家与会话列表** — 按「公屏 → 在线玩家 → 最近离线」排序，玩家卡带真实 ID、皮肤头像、在线/离线状态点与未读红点；私聊走 `/msg`，分会话存储草稿与滚动位置，离线/屏蔽只读
- 🖥️ **玩家档案页** — 从聊天或会话列表推入玩家档案：数据总览、复制按钮、角色标识；聊天与档案之间整页推入转场
- 🎨 **Skia 矢量界面与动效** — 圆角气泡、模糊背景、滚动和 QQ 风格的滑入动画均为矢量绘制
- 🧭 **手机式导航** — 底部三个标签页（聊天 / 个人 / 设置）与页面级推入/弹出转场，公屏与私聊之间同款切换动画
- 📱 **自定义壁纸 / 头像** — 上传本地图片，裁剪后替换界面背景 / 玩家头像；同一服务器装了 AtomChat 时头像自动同步给其他玩家
- 🚫 **屏蔽管理** — 设置内可视化屏蔽名单（头像 + 名字 + 一键取消）；「隐藏被屏蔽玩家的消息」可关，关掉后公屏仍可见但私聊保持屏蔽
- ⚙️ **高度自定义** — 游戏内设置页（磁贴主页：外观 / 聊天 / 隐私与屏蔽 / 关于），外观 / 颜色 / 偏好等均可配置，全部即时生效并立即写盘
...
- ⏱️ **时间戳分隔与跨消息选择** — 首条消息始终显示时间戳胶囊，之后按可配置间隔显示；在消息列表上跨多条消息拖选文字，Ctrl+C 一次复制多段
- 🌐 **其他开关** — 图片接收开关（关闭后显示绿色 `[图片]` 占位）、传送模式循环（`/tp` / `/tpa` / `auto`）、IMBlocker 命令态桥接与 WATUT 输入中指示
...
**路径** — `.minecraft/config/atomchat/` 下存放 `atomchat-client.json` 与各数据子目录：
...
**配置文件** — 直接编辑 `atomchat-client.json`（手动修改后需重启游戏）。常用键：
